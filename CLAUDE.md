...
4. **Insider flow** — `discovery/strategies/insider_flow_strategy.py` `InsiderFlowPositionStrategy` (`insider_flow_form4`, long-only): cumulative Form 4 buys ≥ threshold in last `lookback` days AND close > EMA; exit on EMA cross. Grid: insider_threshold [$50k,$100k,$250k] × lookback [3,5,7] × ema_period [20,50] (18). Needs a per-bar `insider_buy_value` column — **now supplied (Task 5)** by `discovery/data_feeds/edgar_historical.attach_insider_buy_value` via `enrich()` (gated by `DISCOVERY_INSIDER_FEED_ENABLED`). History is PARTIAL/rate-limited, so the family validates only where Form 4 data exists; fail-open to all-flat otherwise.
...
6. **PEAD earnings drift** — `discovery/strategies/pead_strategy.py` `PEADPositionStrategy` (`pead_earnings_drift`): N days after an earnings announcement, LONG if surprise > +threshold & close > EMA, SHORT if surprise < −threshold & close < EMA; hold ≤ hold_days, exit early on 2×ATR adverse move. Needs per-bar `earnings_surprise` (FMP) via `enrich()`. Grid: surprise_threshold [3%,5%,10%] × entry_delay_days [1,2,3] × hold_days [15,20,30] × ema_period [10,20] = **54 combos**. Gated by `DISCOVERY_PEAD_ENABLED`. Tests: `discovery/test_pead_strategy.py`.
7. **Short-interest momentum** — `discovery/strategies/short_interest_momentum_strategy.py` `ShortInterestMomentumPositionStrategy` (`short_interest_momentum`): SHORT when SI rising >threshold on RSI>55 & EMA50<EMA200; LONG (squeeze) when SI falling sharply on RSI<65 & EMA50>EMA200 & >2% up-day on above-avg volume. Needs per-bar `si_change` (FINRA WoW) via `enrich()`. Grid: short_increase_threshold [10%,15%,20%] × short_decrease_threshold [15%,20%,25%] × rsi_period [10,14] × volume_multiplier [1.3,1.5,2.0] = **54 combos**. Gated by `DISCOVERY_SHORT_MOMENTUM_ENABLED`. Tests: `discovery/test_short_interest_momentum_strategy.py`.
8. **Sector rotation** — `discovery/strategies/sector_rotation_strategy.py` `SectorRotationPositionStrategy` (`sector_rotation`): rank 11 GICS sector ETFs by RS; LONG a stock leading a top-N sector / SHORT a stock lagging a bottom-N sector, gated by sector-ETF volume confirmation. Needs `sector_rank_{10,20,30}`/`sector_ret_{10,20,30}`/`sector_vol_ratio` (Alpaca sector ETFs) via `enrich()`. Grid: rs_period [10,20,30] × top_n_sectors [2,3,4] × volume_threshold [1.1,1.2,1.5] = **27 combos**. Gated by `DISCOVERY_SECTOR_ROTATION_ENABLED`. Tests: `discovery/test_sector_rotation_strategy.py`.
